    ...
    return async function handle(req) {
        ...
        try {
            ...
            if (!error.isWireError) {
                (0, protocol_headers_js_1.mergeNonProtocolHeaders)(context.responseHeader, error.metadata);
            }
...
function createStreamHandler(opt, spec, serialization, endStreamSerialization) {
    return async function handle(req) {
        ...
        // We run the request gate before receiving the body, so that a request it
        // rejects is never read, decompressed, or parsed.
        const it = await (0, invoke_implementation_js_1.applyRequestGate)(context, opt.requestGate, () => (0, invoke_implementation_js_1.transformInvokeImplementation)(spec, context, opt.interceptors)(inputIt));
